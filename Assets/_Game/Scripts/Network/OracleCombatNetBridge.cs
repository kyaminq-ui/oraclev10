...
public class OracleCombatNetBridge : MonoBehaviour
{
    ...
    /// <summary>Envoie un message de chat ; diffuse via RPC en ligne, sinon applique localement.</summary>
    public void TrySubmitChat(string senderName, string message)
    {
        if (string.IsNullOrEmpty(message)) return;

        if (!ShouldSendCommandsOverNetwork)
        {
            CombatLog.AppendChat(senderName, message);
            return;
        }

        _pv.RPC(nameof(RpcApplyChat), RpcTarget.All, senderName, message);
    }

    [PunRPC]
    void RpcApplyChat(string senderName, string message)
    {
        CombatLog.AppendChat(senderName, message);
    }
