...
public static class OracleHudRuntimeSprites
{
    ...
    /// <summary>Frames PNG extraites de <c>animated_timer_icon.gif</c> (<c>Tools → Oracle → Extract …</c> ou dossier équivalent).</summary>
    public const string AnimatedTimerIconResourcesPath = "OracleHUD/Frames/animated_timer_icon";

    public const float DefaultTimerIconAnimFps = 10f;
    ...
    public static Sprite LoadPassiveIcon() =>
        LoadSpriteFlexible("UI_ICONE_PASSIF", "ui_icone_passif", "passive_icon_hud");

    /// <summary>
    /// Séquence <see cref="AnimatedTimerIconResourcesPath"/> si des sprites existent ; sinon <see cref="LoadTimerIcon"/>.
    /// </summary>
    public static void ApplyAnimatedTimerOrFallback(Image icon, float fps = DefaultTimerIconAnimFps, bool useUnscaledTime = true)
    {
        if (icon == null) return;
        var frames = CombatAnimationResources.LoadAllSpritesSorted(AnimatedTimerIconResourcesPath);
        if (frames != null && frames.Length > 0)
        {
            var anim = icon.GetComponent<HudAnimatedStatIcon>();
            if (anim == null) anim = icon.gameObject.AddComponent<HudAnimatedStatIcon>();
            anim.resourcesFolder = AnimatedTimerIconResourcesPath;
        ...
        var orphan = icon.GetComponent<HudAnimatedStatIcon>();
        if (orphan != null)
    ...
    public static void ApplyCombatHudIfMissing(Image paIcon, Image pmIcon, TimerUI timer)
    {
        ...
        if (timer != null && timer.timerIconImage != null)
            ApplyAnimatedTimerOrFallback(timer.timerIconImage, DefaultTimerIconAnimFps, useUnscaledTime: false);
    }
    ...
    public static void ApplyPassiveTimerIfMissing(Image timerIcon)
    {
        ...
        ApplyAnimatedTimerOrFallback(timerIcon, DefaultTimerIconAnimFps, useUnscaledTime: true);
    }
}
