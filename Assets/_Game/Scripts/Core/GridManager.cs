...
public class GridManager : MonoBehaviour
{
    ...
    // =========================================================
    // MURS TEMPORAIRES (Pilier / Balise)
    // =========================================================
    public void TickTemporaryWalls()
    {
        ...
        for (int x = 0; x < config.width; x++)
            ...
            {
                Cell c = grid[x, y];
                if (c == null || c.TemporaryWallTurns <= 0) continue;
                c.TemporaryWallTurns--;
                if (c.TemporaryWallTurns <= 0)
                {
                    c.IsWalkable = true;
                    CombatLog.Append("Un mur temporaire se dissipe.");
                }
            }
    }
