...
public class TileSpriteRegistry : ScriptableObject
{
    ...
    public Sprite GetRandomObstacleTile(System.Random rng)
    {
        ...
        {
            if (!IsExcludedProceduralObstacle(obstacleTiles[i]))
                eligible++;
        }
        ...
        for (int i = 0; i < obstacleTiles.Length; i++)
        {
            var sprite = obstacleTiles[i];
            if (IsExcludedProceduralObstacle(sprite))
                continue;
    ...
    static bool IsExcludedProceduralObstacle(Sprite sprite)
    {
